...
test("checks all fields and buttons are present on create product page and the back buttons work", async ({page}) => {
  ...
  await expect(
    container.getByRole("heading", { name: "Opret produkt", level: 1 })
  ).toBeVisible();
  await expect(
    container.getByRole("button", { name: "Tilbage til produkter" })
  ).toBeVisible();
  ...
  await expect(
    container.getByRole("button", { name: "Annuller" })
  ).toBeVisible();
  ...
  await expect(
    page.locator('textarea:visible, [role="textbox"]:visible')
  ).toHaveCount(1);
...
test("Shows error messages when not all requred fields have been filled out", async ({page}) => {
  ...
  await expect(page.locator('input[name="name"]')).toHaveClass(
    /border-red-500/
  );

  //price
  await expect(page.getByText("Skal udfyldes").nth(1)).toBeVisible();
  await expect(page.locator('input[name="price"]')).toHaveClass(
    /border-red-500/
  );

  //category
  await expect(page.getByText("Vælg en kategori")).toBeVisible();
  await expect(page.getByRole("combobox").first()).toHaveClass(
    /border-red-500/
  );
  ...
  await expect(
    page.getByText("Udfyld venligst alle påkrævede felter.")
  ).toBeVisible();
});
...
test("Default values are shown when nonrequired fields arent shown", async ({page}) => {
  ...
  await expect(
    page.getByRole("img", { name: "defaultTest" }).nth(1)
  ).toBeVisible();
});
