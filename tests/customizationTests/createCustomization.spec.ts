...
test("Displays a 'Create customization' button on the customization list", async ({page}) => {
  ...
  await expect(
    page.getByRole("link", { name: "Opret ny tilpasning" })
  ).toBeVisible();
...
test("Submitting the form returns to /customizations and shows success notification", async ({page}) => {
  ...
  await expect(page.getByText(/Tilpasning er oprettet/i)).toBeVisible();

  await expect(page.getByText("Ny kategori")).toBeVisible();

  const summary = page
    .getByRole("button", { name: /Ny kategori/i })
    .or(page.getByText("Ny kategori"));
  await summary.click();
...
test("Shows validation errors when required fields are missing", async ({page}) => {
  ...
  await expect(
    page
      .getByText(/Titel/i)
      .locator("..")
      .getByText(/Titel er påkrævet/i)
  ).toBeVisible();

  // Because no options were added:
  await expect(
    page.getByText(/Du skal tilføje mindst én mulighed/i)
  ).toBeVisible();

  //general error
  await expect(
    page.getByText(/Udfyld venligst alle krævede felter./i)
  ).toBeVisible();
